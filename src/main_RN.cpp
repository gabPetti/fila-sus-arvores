...
void lerArquivoInternacoes(burbTree<internacao>& arvore, tabelaHash& indice, string nomeArquivo){
...
void executarBenchmarkRN(burbTree<internacao>& arvore, tabelaHash& indice) {
    ...
    if (arquivoSaida.is_open()) {
        ...
        arquivoSaida.close();
        cout << "-> Resultados salvos com sucesso em 'benchmark_rn.csv'\n";
    } else {
        cout << "-> ERRO: Nao foi possivel criar o arquivo 'benchmark_rn.csv'\n";
    }
}

int main(){
    burbTree<internacao> arvore;
    tabelaHash indice;
    int opcao;
    ...
    do {
        ...
        switch(opcao){
            ...
            case 11: executarBenchmarkRN(arvore, indice); break;
